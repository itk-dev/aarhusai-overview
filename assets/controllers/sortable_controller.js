...
export default class extends Controller {
    ...
    connect() {
        ...
        if (this.headerTargets.length > 0 && this.bodyTargets.length > 0) {
            ...
            const arrow = this.headerTargets[0].querySelector('[data-sort-arrow]');
    ...
    _sortByColumn(index, ascending) {
        ...
        const sorted = [...bodies].sort((a, b) => {
            const aText = a.querySelector('tr td:nth-child(' + (index + 1) + ')').textContent.trim().toLowerCase();
            const bText = b.querySelector('tr td:nth-child(' + (index + 1) + ')').textContent.trim().toLowerCase();
